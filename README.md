...
- [Additional configuration](#additional-configuration)
  ...
  - [API keys IP allowlist](#api-keys-ip-allowlist)
...
- Kubernetes **>= 1.32 && <= 1.34** (Supported versions mean that installation passed for the versions listed; however, it **may** work on older k8s versions as well)
- Helm **3.8.0+**
...
> **Note:** Default username and password is: AdminCF/AdminCF. You must change the default password via Admin Panel immediately after installation.
...
cfapi:
  env:
    # e.g. 3 = an extra L7 proxy of your own in front of the ingress
    TRUSTED_PROXY_COUNT: "3"
```

> [!IMPORTANT]
> Count only proxies that append to `X-Forwarded-For`: the chart's ingress controller (or ALB) and the `internal-gateway`. Add `1` for every L7 proxy of your own in front of the ingress, and set `use-forwarded-headers: "true"` in the Ingress-NGINX configuration so the header is passed through. TCP/L4 load balancers do not append to the header and must not be counted.

3. Verify the derived address **before** turning the allowlist on — from an admin workstation and from every network that runs builds (runtimes, runners, external CI):

```bash
curl https://<your-onprem-domain>/api/whereami
# {"ip":"<the caller's public egress IP>"}
```

If the returned IP is an internal address (ingress or pod IP), `TRUSTED_PROXY_COUNT` is too low. If it is not the caller's real egress address, the value is too high and the derived IP is client-forgeable. In both cases fix the value first — arming the allowlist with a wrong count blocks legitimate API keys (including build runtimes) or makes the check spoofable.
